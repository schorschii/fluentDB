...
<div id='container'>
	...
	<div id='header'>
		<span class='left'>
			...
			<a href='index.php' onclick='event.preventDefault();refreshContentExplorer("views/homepage.php");' class='title'><?php echo LANG('project_name'); ?></a>
		...
		<span id='search-container' onfocusout='if(!this.contains(event.relatedTarget)) {closeSearchResults()}'>
			<input type='text' id='txtGlobalSearch' autocomplete='off' placeholder='<?php echo LANG('search_placeholder'); ?>' onfocus='openSearchResults()' onkeyup='if(event.keyCode==27) {closeSearchResults()} else if(event.keyCode==40) {focusNextSearchResult()} else {doSearch(this.value)}' onpaste='doSearch(this.value)'></input>
